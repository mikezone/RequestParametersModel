...
- (instancetype)initWithQueryString:(NSString *)queryString {
    if (self = [super init]) {
        NSDictionary *parameter = [queryString convertToDictionary];
...
- (NSString *)queryString {
    return [[self parameters] convertToQueryString];
}
...
- (void)setterHandlerWithClass:(Class)aClasss pName:(NSString *)pName invocation:(NSInvocation *)anInvocation {
    ...
    if (typeEncoding == MKObjCTypeClass) {
        void *value;
        [anInvocation getArgument:&value atIndex:2];
        [self setValue:(__bridge id _Nonnull)(value) forKey:pName];
        return;
    }
    switch (typeEncoding) {
        case MKObjCTypeChar:
